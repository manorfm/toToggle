...
const (
	...
	// ActivationRuleTypeParameter era "attribute" até o protótipo v2.6.4 renomear o tipo (não só
	// o rótulo de UI — o próprio identificador interno, confirmado via
	// get_component_data("EditDrawer")). Breaking change deliberado, decidido com o usuário — ver
	// a migração 20260914000000_rename_attribute_rule_type_to_parameter.sql, que reescreve
	// qualquer regra já persistida com rule_type='attribute'.
...
func (ar *ActivationRule) ValidateRule() error {
	...
	case ActivationRuleTypeParameter, ActivationRuleTypeUserID, ActivationRuleTypeIP, ActivationRuleTypeCountry:
		if ar.Value == "" {
			return fmt.Errorf("valor da regra é obrigatório")
		}
	case ActivationRuleTypeCohort:
		// v2.6.4: cohort deixou de comparar Value contra uma lista — confirmado no protótipo
		// ("No value needed, just turn it on for this cohort"). Value não é mais validado (nem
		// exigido, nem rejeitado); a avaliação real vira uma checagem de presença/não-vazio do
		// context_key "cohort" no SDK (ver strategy/cohort em cada client). Uma regra antiga
		// gravada com Value tipo "canary,beta" continua passando aqui — o dado só fica inerte.
...
func validContextKey(ruleType ActivationRuleType, key string) bool {
	if strings.HasPrefix(key, "attributes.") {
		return len(strings.TrimPrefix(key, "attributes.")) > 0 &&
			(ruleType == ActivationRuleTypePercentage || ruleType == ActivationRuleTypeParameter)
	}
...
func GetRuleTypeOptions() map[ActivationRuleType]string {
	return map[ActivationRuleType]string{
		...
		ActivationRuleTypeParameter:  "Parameter - Ativar baseado em valor de contexto",
		...
		ActivationRuleTypeCohort:     "Cohort - Ativar quando o app marcar a requisição com um cohort (sem lista de valores)",
	}
}
