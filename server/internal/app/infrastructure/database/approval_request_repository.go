...
func (r *approvalRequestRepository) GetWithDetails(ctx context.Context, id string) (*entity.ApprovalRequestWithDetails, error) {
	...
	query := `
		SELECT 
			ar.*,
			COALESCE(ar.requester_name, u_req.username, '') as requester_name,
			COALESCE(ar.team_name, t.name, '') as team_name,
			COALESCE(ar.application_name, app.name, '') as application_name,
	...
	err := row.Scan(
		...
		&approvalRequest.TeamName,
		&approvalRequest.ApplicationName,
...
func (r *approvalRequestRepository) GetAllWithDetails(ctx context.Context) ([]*entity.ApprovalRequestWithDetails, error) {
	...
	query := `
		SELECT 
			...
			ar.requester_name, ar.team_name, ar.application_name,
			COALESCE(ar.requester_name, u_req.username, '') as requester_name,
			COALESCE(ar.team_name, t.name, '') as team_name,
			COALESCE(ar.application_name, app.name, '') as application_name,
			COALESCE(tgl.path, '') as toggle_path,
